...
def process_directory(directory, model, transform, output_dir, resume_from=0, start_time=None):
    image_paths = load_patches_from_directory(directory)
    dataset = PatchDataset(image_paths, transform=transform)
    batch_size = get_optimal_batch_size()
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)
    extract_features_and_save(model, dataloader, output_dir, batch_size, resume_from=resume_from, start_time=start_time)

if __name__ == "__main__":
    # Define paths
    normal_dir = '/data/ja235/camelyon16_project/data/test_patches/normal'
    tumor_dir = '/data/ja235/camelyon16_project/data/test_patches/tumor'
    output_dir_normal = '/data/ja235/camelyon16_project/test_extracted_features/normal'
    output_dir_tumor = '/data/ja235/camelyon16_project/test_extracted_features/tumor'
    ...
    # Process normal patches
    print("Processing normal patches...")
    resume_from, start_time = load_progress(output_dir_normal)
    process_directory(normal_dir, model, transform, output_dir_normal, resume_from=resume_from, start_time=start_time)

    # Process tumor patches
    print("Processing tumor patches...")
    resume_from, start_time = load_progress(output_dir_tumor)
    process_directory(tumor_dir, model, transform, output_dir_tumor, resume_from=resume_from, start_time=start_time)
