...
# Load the merged CSV for training set
merged_csv_path = "/data/ja235/camelyon16_project/merged_patch_probabilities.csv"
df = pd.read_csv(merged_csv_path)

# Create new output directories for training set results
output_dir = "/data/ja235/camelyon16_project/training_results_new"
...
# Function to determine label based on slide_id
def get_label(slide_id):
    return 1 if slide_id.startswith('tumor') else 0
...
for slide_id in tqdm(slide_ids, desc="Processing slides"):
    try:
        ...
        features['label'] = get_label(slide_id)  # Determine label based on slide_id
...
slide_features_df.to_csv(os.path.join(output_dir, 'train_slide_level_features.csv'), index=False)

print("Feature extraction completed. Results saved to:")
print(f"1. Heatmaps: {heatmap_dir}/<slide_id>_heatmap.png")
print(f"2. Features: {os.path.join(output_dir, 'train_slide_level_features.csv')}")